...
function [figure_handle,count,speeds,directions,Table,Others] = wind_rose(direction,speed,varargin)
    ...
    % 2022/Aug/14 - Thanks to JoséMiguel Jáuregui and NAXYA for funding the following changes
...
if ~analysis
    [color,axs,PTCH] = DrawPatches(N,n,vwinds,count,colorfun,figcolor,min_radius,colors,inverse,axs,colormap_set,escala,CenterLocation,LineWidth,EdgeColorModified,EdgeColor,gap,EdgeAlpha,FaceAlpha); % Draw the windrose, knowing the angles, the range for each direction, the speed ranges, the count (frequency) values, the colormap used and the colors used.
    if ~CenterLocation.S
    ...
    FrequencyLabels(circles,radius,FreqLabelAngle,escala,CenterLocation,min_radius,frequencyFontSize,frequencyFontName,frequencyFontWeight,frequencyFontColor,frequencyFontAngle);      % Display frequency labels
    ...
    if ruler~=false
        ...
        for i=1:length(circle_lab)
            ...
            text(K*circle_pos(i),0.05*max(circle_pos),[num2str(circle_lab(i)) '%'],'horizontalalignment','center','verticalalignment','bottom','fontname',frequencyFontName,'fontsize',frequencyFontSize,'fontweight',frequencyFontWeight,'fontangle',frequencyFontAngle,'color',frequencyFontColor);
        end
    end
...
function [color,axs,PTCH] = DrawPatches(N,n,vwinds,count,colorfun,figcolor,min_radius,colors,inverse,axs,colormap_set,escala,CenterLocation,LineWidth,EdgeColorModified,EdgeColor,gap,EdgeAlpha,FaceAlpha)
    if isempty(axs)
...
%         plot(axs,0+CenterLocation.X,0+CenterLocation.Y,'.','color',figcolor,'markeredgecolor',figcolor,'markerfacecolor',figcolor); % This will create an empty legend entry.
    ...
    for i=1:length(N)                                                      % For every angle
        for j=length(vwinds):-1:1                                          % For every wind speed range (last to first)
            ...
            try
                PTCH(i,j) = fill(x*escala+CenterLocation.X,y*escala+CenterLocation.Y,color(j,:),'edgecolor',EdgeCol,'linewidth',LineWidth,'edgealpha',EdgeAlpha,'facealpha',FaceAlpha); % Draw them in the specified color. 
            catch exc
                ...
                PTCH(i,j) = fill(x*escala+CenterLocation.X,y*escala+CenterLocation.Y,color(j,:),'edgecolor',EdgeCol,'linewidth',LineWidth); % Same but with no alpha properties for those Matlab versions that do not support alpha.
            end
        end
    end
end
function FrequencyLabels(circles,radius,angulo,escala,CenterLocation,rmin,frequencyFontSize,frequencyFontName,frequencyFontWeight,frequencyFontColor,frequencyFontAngle)
    ...
    for i=1:length(circles)
        text(radius(i)*c*escala+CenterLocation.X,radius(i)*s*escala+CenterLocation.Y,[num2str(circles(i)) '%'],'HorizontalAlignment',ha,'verticalalignment',va,'fontsize',frequencyFontSize,'fontname',frequencyFontName,'fontsize',frequencyFontSize,'fontweight',frequencyFontWeight,'fontangle',frequencyFontAngle,'color',frequencyFontColor); % display the labels for each circle
    end
%     if length(radius)>1
%         rmin = radius(1)-abs(diff(radius(1:2)));
        if rmin>0
            ...
            text(rmin*c*escala+CenterLocation.X,rmin*s*escala+CenterLocation.Y,'0%','HorizontalAlignment',ha,'verticalalignment',va,'fontsize',frequencyFontSize,'fontname',frequencyFontName,'fontsize',frequencyFontSize,'fontweight',frequencyFontWeight,'fontangle',frequencyFontAngle,'color',frequencyFontColor); % display the labels for each circle
        end
%     end
end
...
function CardinalLabels(circlemax,labels,lots_of_labels,escala,CenterLocation,axisFontSize,axisFontName,axisFontWeight,axisFontColor,axisFontAngle)
    if ~lots_of_labels && isstruct(labels)
        ...
        text(0+CenterLocation.X, circlemax*escala+CenterLocation.Y,labels.N      ,'HorizontalAlignment','center','verticalalignment','bottom','fontsize',axisFontSize,'fontname',axisFontName,'fontweight',axisFontWeight,'color',axisFontColor,'fontangle',axisFontAngle); % North label
        text(0+CenterLocation.X,-circlemax*escala+CenterLocation.Y,labels.S      ,'HorizontalAlignment','center','verticalalignment','top'   ,'fontsize',axisFontSize,'fontname',axisFontName,'fontweight',axisFontWeight,'color',axisFontColor,'fontangle',axisFontAngle); % South label
